...
function SimTimeStep( dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution )
{
	...
	for(var i = 0; i < springs.length; i++){
		...
		let pp_0 = positions[p0];
		let pp_1 = positions[p1];
		let pv_0 = velocities[p0];
		let pv_1 = velocities[p1]; 

		let spring_length = pp_1.sub(pp_0).len();
		let spring_direction = pp_1.sub(pp_0).div(spring_length);

		forces[p0] = forces[p0].add(spring_direction.mul(stiffness * (spring_length - spring.rest)));
		forces[p1] = forces[p1].add(spring_direction.mul(stiffness * (spring.rest - spring_length)));

		forces[p0] = forces[p0].add(spring_direction.mul(pv_1.sub(pv_0).dot(spring_direction)).mul(damping));
		forces[p1] = forces[p1].add(spring_direction.mul(pv_0.sub(pv_1).dot(spring_direction)).mul(damping));
	...
	for (var i = 0; i < positions.length; i++) {
		...
		velocities[i] = velocities[i].add((forces[i].div(particleMass).add(gravity)).mul(dt));
	...
	for(let i = 0; i < positions.length; i++){
		...
		if (position.x < -1.0) { 
			let h = - 1.0 - position.x;
			position.x = restitution * h - 1;
			velocity.x = - velocity.x * restitution;
		}
		if (position.x > 1.0) {	
			let h = position.x - 1.0;
			position.x = 1.0 - restitution * h;
			velocity.x = - velocity.x * restitution;
		}

		if (position.y < -1.0) {
			let h = - 1.0 - position.y;
			position.y = restitution * h - 1;
			velocity.y = - velocity.y * restitution;
		}
		if (position.y > 1.0) {
			let h = position.y - 1.0;
			position.y = 1.0 - restitution * h;
			velocity.y = - velocity.y * restitution;
		}

		if (position.z < -1.0) {
			let h = - 1.0 - position.z;
			position.z = restitution * h - 1;
			velocity.z = - velocity.z * restitution;
		}
		if (position.z > 1.0) {
			let h = position.z - 1.0;
			position.z = 1.0 - restitution * h;
			velocity.z = - velocity.z * restitution;
		}
	}
}
